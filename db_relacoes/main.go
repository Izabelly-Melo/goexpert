...
type Product struct {
	...
	SerialNumber SerialNumber // relação um-para-um com SerialNumber (has one)
...
func main() {
	...
	*/
	...
	products := []Product{}
	db.Preload("Category").Preload("SerialNumber").Find(&products) // Preload carrega a categoria e o número de série associados a cada produto
	for _, p := range products {
		println(p.Name, p.Category.Name, p.SerialNumber.Number)
	}
